  ...
  <body>
      ...
      <div class="row-fluid">
        ...
        <div class="span9 crud">
          ...
          <table class="table table-striped table-bordered">
            <thead>
              <th></th>
              <th>Current Version saved on <?php echo $programmesetting->created_at; ?></th>
              <th>Revision created on <?php echo $revision->created_at ; ?></th>
            </thead>
            <tbody>
              <?php foreach ($old as $field => $value) : ?>
              <tr>
                <td><?php echo (!array_key_exists($field, $attributes)) ? __("programmesettings.$field") : $attributes[$field] ?></td>
          ...
          <div class="form-actions">
            <a class="btn btn-danger promote_toggler" href="#promote_revision" rel="<?php echo  action(URI::segment(1).'/'.URI::segment(2).'/programmesettings@promote', array($revision->id))?>">Accept Changes And Promote To Live</a>
            <a class="btn btn-secondary" href="<?php echo url(URI::segment(1).'/'.URI::segment(2).'/programmesettings')?>">Ignore For Now</a>
          </div>
        </div><!-- span9 -->
      </div>
    </div> <!-- /container -->
    <div class="modal hide fade" id="promote_revision">
            ...
            <div class="modal-footer">
              <?php echo Form::open('programmesettings/promote', 'POST')?>
