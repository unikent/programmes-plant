  ...
  <tbody <?php echo $field_type == 'programmes' ? 'class="sortable-tbody"' : ''; ?>>
    ...
    <tr id="field-id-<?php echo $subject->id ?>">
      ...
      <td>
        ...
        <a class="btn btn-primary" href="<?php echo url($type.'/fields/'.$field_type.'/edit/'.$subject->id);?>"><?php echo __('fields.btn.edit'); ?></a>

        <?php if($subject->active == 1 ): ?>
          <a class="btn btn-danger" href='<?php echo url($type.'/fields/'.$field_type.'/deactivate');?>?id=<?php echo $subject->id;?>'><?php echo __('fields.btn.deactivate'); ?></a>
        <?php else: ?>
          <a class="btn btn-success" href='<?php echo url($type.'/fields/'.$field_type.'/reactivate');?>?id=<?php echo $subject->id;?>'><?php echo __('fields.btn.reactivate'); ?></a>
        <?php endif; ?>
