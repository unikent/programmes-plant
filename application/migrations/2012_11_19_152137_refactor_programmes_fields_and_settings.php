...
class Refactor_Programmes_Fields_And_Settings {
	...
	public function addPregrameFields(){
		...
		$programme_fields[] = array('Award', 'select', '', ''); //taken from awards table. Single choice. 
		$programme_fields[] = array('Awarding institute or body', 'select', 'University of Kent', '');//options: University of Kent, Medway School of Pharmacy, University of Kent and Canterbury Christ Church University. Single choice. University of Kent as the default.
		$programme_fields[] = array('Honours type', 'select', '', '');//options: Single honours, joint honours, European honours
		$programme_fields[] = array('Administrative school', 'text', 'If the programme is joint honours, please select the school with the main administrative responsibility.', '');//taken from schools table
		$programme_fields[] = array('Additional school', 'select', 'For joint honours programmes, please select the other school that this programme is run in conjunction with.', '');//taken from schools table
		$programme_fields[] = array('Subject area 1', 'select', 'Please select the main subject area for this programme.  If the programme is joint honours, please ensure the subject selected is taught by the administrative school.', '');//taken from subjects table
		$programme_fields[] = array('Subject area 2', 'select', 'If the programme is joint honours, please select the second subject area.', '');//taken from subjects table
		$programme_fields[] = array('UCAS code', 'text', '', '');
		$programme_fields[] = array('Location', 'select', '', ''); //taken from campus table.  Single choice.
		$programme_fields[] = array('Mode of study', 'select', '', '');//options: Full-time only, Part-time only, Full-time or part-time
		$programme_fields[] = array('Attendance mode', 'checkbox', '', '');//options: Campus, Distance with attendance, Distance without attendance, Mixed. Default: Campus
		$programme_fields[] = array('Attendance pattern', 'checkbox', '', '');//options: Day-time, evening, weekend, customized. Default: Day-time
		...
		$programme_fields[] = array('Funding', 'textarea', 'Please note standard text will appear about funding and scholarships. See ‘Standard messages’ (pdf). If the standard text is not applicable, please enter alternative text here.', ''); // Function: Overrides the standard message. 


		//Section: How to apply 
		$programme_fields[] = array('How to apply', 'textarea', 'Please note standard text will appear about how to apply. See ‘Standard messages’ (pdf). If the standard text is not applicable, please enter alternative text here.', ''); // Function: Overrides the standard message. 

		//Section: Further information 
		$programme_fields[] = array('Enquiries', 'textarea', 'Please note standard text will appear about Enquiries.  See ‘Standard messages’ (pdf). If the standard text is not applicable, please enter alternative text here.', ''); // Function: Overrides the standard message. 
		$programme_fields[] = array('Subject leaflet', 'select', 'Please select the subject leaflet that contains information about this programme.', ''); //Type: predefined selection (subject leaflet table needs to be built) 
		$programme_fields[] = array('Subject leaflet 2', 'text', 'If the programme is joint honours, please select an additional subject leaflet that contains information about this programme.', ''); //Type: predefined selection
		...
		$programme_fields[] = array('Subject categories', 'text', 'select the subject categories this programme falls into. The programme will appear under these category in the course finder. ', ''); //Type: Subject category multi-selection tool. Predefined subject categories. Function:Read only to users, editable by superusers. 
		...
		foreach ($programme_fields as $field) {
			$this->add_field('ProgrammeField', 'programmes', $field[0], $field[1], $field[2], $field[3]);
		}

		// Add the fields in
		foreach ($programme_setting_fields as $field) {
			$this->add_field('ProgrammeSettingField', 'programme_settings', $field[0], $field[1], $field[2], $field[3], true);
		}
	}
	 ...
	 */
	public function add_field($modelname, $tablename, $title, $type, $hints, $options, $isglobal = false)
	{
        ...
        $field_object->view = 1;
        if($isglobal){
        	$field_object->isglobal = $isglobal;
        }
        $field_object->save();
